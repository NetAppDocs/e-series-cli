...
---
= Create RAID volume (manual drive select) - SANtricity CLI
:icons: font
:imagesdir: ../media/
...
NOTE: The character limit for standard and thin volume names is 60 characters. The volume name can contain any combination of alphanumeric characters, underscore (_), hyphen (-), and pound (#).
...
a|
If this parameter is set to `TRUE`, only DULBE-capable drives will be used to create disk pools or volume groups. If the parameter is set to `FALSE` or not provided, no checking for DULBE capability is performed. If all the drives selected for the volume group or disk pool are DULBE-capable, then the storage array level flag determines whether the volume group or disk pool is created as resource provisioned.
...
You can use any combination of alphanumeric characters, underscore (_), hyphen (-), and pound (#) for the names. Standard and thin volume names can have a maximum of 60 characters; all other component names can have a maximum of 30 characters.
