...
The following table list changes to the CLI and Script Commands for release 12.10.

[cols="2*",options="header"]
|===
| Command| Description of Change
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/set-storagearray-host-io-protocol.html[Set Storage Array Host I/O Protocol]
a|
Added `nvmetcp` option to the command.
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/create-raid-volume-automatic-drive-select.html[Create RAID volume (automatic drive select)]
a|
Increased character limit for the `userLabel` parameter from 30 to 60 characters. 

NOTE: Downgrade from SANtricity 12.10 to earlier versions is not supported if the `userLabel` parameter exceeds 30 characters.
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/create-raid-volume-free-extent-based-select.html[Create RAID volume (free extent based select)]
a|
Increased character limit for the `userLabel` parameter from 30 to 60 characters.

NOTE: Downgrade from SANtricity 12.10 to earlier versions is not supported if the `userLabel` parameter exceeds 30 characters.
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/create-raid-volume-manual-drive-select.html[Create RAID volume (manual drive select)]
a|
Increased character limit for the `userLabel` parameter from 30 to 60 characters.

NOTE: Downgrade from SANtricity 12.10 to earlier versions is not supported if the `userLabel` parameter exceeds 30 characters.
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/create-volume-diskpool.html[Create volume in disk pool]
a|
Increased character limit for the `userLabel` parameter from 30 to 60 characters.

NOTE: Downgrade from SANtricity 12.10 to earlier versions is not supported if the `userLabel` parameter exceeds 30 characters.
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/recover-volume.html[Recover RAID volume]
a|
Increased character limit for the `userLabel` parameter from 30 to 60 characters.
...
a|
The character limit for standard and thin volumes names has been increased from 30 to 60 characters.

|===

== SANtricity 12.00

The following table list changes to the CLI and Script Commands for release 12.00.
...
Added command.
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/show-storage-array-host-io-protocol.html[Show Storage Array Host I/O Protocol Information]
a|
Added command. 
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/start-storagearray-externalkeymanagement-test.html[Test external key management communication]
a|
Added test KMS communication results to command output. 
a|
https://docs.netapp.com/us-en/e-series-cli/commands-a-z/set-storagearray-managementinterface.html[Set storage array management interface]
a|
Removed command. 
