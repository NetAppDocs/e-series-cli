...
As with previous versions of the SMcli, the SMcli downloadable through the SANtricity System Manager has a unique set of parameters. For information on using command line parameters for SANtricity OS 11.53 and older releases, see xref:./get-started/command-line-parameters.adoc[Legacy command line parameters]
